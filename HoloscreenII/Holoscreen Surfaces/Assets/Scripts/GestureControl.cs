...
public class GestureControl : MonoBehaviour {
	...
	private GameObject palm, indexfinger, indexfinger_bone1, indexfinger_bone2, pinkyfinger, pinkyfinger_bone1, pinkyfinger_bone2;
	...
	void Start () {
		...
		pinkyfinger = this.transform.GetChild (4).gameObject;
		pinkyfinger_bone1 = this.transform.GetChild (4).GetChild (1).gameObject;
		pinkyfinger_bone2 = this.transform.GetChild (4).GetChild (2).gameObject;
	...
	private bool checkPosePointing(){
		...
		bool index_up = ((proj_bone2.y / proj_bone1.y) > 1.015);

		vec_bone1 = pinkyfinger_bone1.transform.position - palm.transform.position;
		vec_bone2 = pinkyfinger_bone2.transform.position - palm.transform.position;
		proj_bone1.y = Vector3.ProjectOnPlane(vec_bone1, palm_plane_norm).magnitude;
		proj_bone2.y = Vector3.ProjectOnPlane(vec_bone2, palm_plane_norm).magnitude;
		bool middle_down = ((proj_bone2.y / proj_bone1.y) < 1.03);

		//Debug.Log ("x: " + proj_bone2.x.ToString("F3") + "  y: " + proj_bone2.y.ToString("F3") + "  z: " + proj_bone2.z.ToString("F3"));
		if (index_up&&middle_down) {
			return true;
		}
