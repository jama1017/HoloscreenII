...
public class FaceTrackingController : MonoBehaviour, WebSocketUnityDelegate {
	...
	public string wsIp = "138.16.97.220";
	public string wsPort = "9999";
	...
	{
		// Create web socket
		webSocket = new WebSocketUnity("ws:" + wsIp + ":" + wsPort, this);
	...
	public void OnWebSocketUnityReceiveMessage (string message)
	{
		...
		if (eyeDistance < 0) {
			...
			transform.localPosition = new Vector3(centroid.x - eyeCentroid.x, centroid.y - eyeCentroid.y, 0) * -0.2f;
		}
