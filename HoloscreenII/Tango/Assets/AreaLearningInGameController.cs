...
public class AreaLearningInGameController : MonoBehaviour, ITangoPose, ITangoEvent, ITangoDepth
{
    ...
    private void _SaveMarkerToDisk()
    {
        ...
        foreach (GameObject obj in m_markerList)
        {
            ...
            temp.m_text = obj.GetComponent<TextMesh>().text;
    ...
    private void _LoadMarkerFromDisk()
    {
        ...
        foreach (MarkerData mark in xmlDataList)
        {
            ...
            temp.GetComponent<TextMesh>().text = mark.m_text;
    ...
    {
        // Get placement, orientation of marker
        Camera cam = Camera.main;
        Vector3 planeCenter;
        Vector3 up;
        Vector3 forward;
		...
		if (!m_pointCloud.FindPlane (cam, touchPosition, out planeCenter, out plane)) {
			...
			cam = Camera.main;
			planeCenter = cam.transform.position + cam.transform.forward;
		} else {
			planeCenter = planeCenter + plane.normal * 0.2f;
		}
